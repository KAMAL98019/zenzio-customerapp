...
//     // ✅ FIXED: Check for exactly 10 digits
//     if (_mobileController.text.trim().length != 10) {
...
//     // ✅ FIXED: Check for exactly 6 digits
...
class _SignupScreenState extends State<SignupScreen> {
  ...
  void initState() {
    super.initState();
    // ✅ Check if coming from OTP screen with verified phone
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final args = ModalRoute.of(context)?.settings.arguments;
      if (args is Map) {
        final phone = args['phone'];
        final countryCode = args['countryCode'];
        final otpVerified = args['otpVerified'] == true;

        if (phone != null && otpVerified) {
          setState(() {
            _mobileController.text = phone.toString();
            _selectedCountryCode = countryCode?.toString() ?? '+91';
            _isOtpVerified = true;
            _isOtpSent = true;
          });
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Phone verified! Please complete your registration.'),
              backgroundColor: Colors.green,
            ),
          );
        }
  ...
  Future<void> _handleSignup() async {
    ...
    try {
      // ✅ CRITICAL FIX: Combine country code with phone for registration
      final fullPhone = '${_selectedCountryCode}${_mobileController.text.trim()}';
      
      print('📦 Registering with phone: $fullPhone');
          ...
          SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                // === Birthday & Gender (only after OTP verified) ===
                if (_isOtpVerified) ...[
                  const SizedBox(height: 16),
                  _buildTextField(
                    controller: _birthdayController,
